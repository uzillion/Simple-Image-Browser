...
class View(QWidget):
    ...
    def loadImages(self):
        for k, v in Models.dbImages.items():
            if k not in self.images:
                self.images = [k] + self.images
        self.images.reverse()
        self.images = Models.urlImages + self.images
        Models.urlImages = []
        print(self.images)
    ...
    def thumbnail_bar(self):     # Creates a List of QLabels and places them in a horizontal Layout
        ...
        for i in range(0, 6):
            ...
            if i != (5):
                pixmap = QPixmap()
                pixmap.loadFromData(urllib.request.urlopen(self.images[i]).read())
    ...
    def shiftLeft(self):            # Shifts the select box and scope of the Thumbnail Bar to the left
        if self.selected_index != 0:         # Shift select box till first image
            ...
            if 5 == 5:       # If there are more than 5 images, shift scope of Thumbnail Bar to left
                ...
                pixmap = QPixmap()
                pixmap.loadFromData(urllib.request.urlopen(self.images[self.circularTraverse(1, "left")]).read())
        ...
        if self.selected_index != 4:      # Shift select box till last image
            ...
            if 5 == 5:      # If there are more than 5 images, shift scope of Thumbnail Bar to rght
                for i in range(0, 4):    # Shifts scope
                    self.getWidget(i).setPixmap(self.getWidget(i+1).pixmap().scaled(((self.W-40)/5)-20, ((self.W-40)/5)-20, Qt.KeepAspectRatio, Qt.SmoothTransformation))
                pixmap = QPixmap()
                pixmap.loadFromData(urllib.request.urlopen(self.images[self.circularTraverse(5, "right")]).read())
...
#=========================================== EVENT HANDLERS =========================================
    ...
    def saveImgs(self):
        for img in self.images:
            if img not in Models.dbImages:      # If no tags exist, create a new list for tags
                Models.dbImages[img] = []
        self.saveTags()
    ...
    def addTag(self):
        ...
        if not (self.tagBox.text().isspace() or self.tagBox.text() == ""):
            ...
            image_key = self.images[(self.image_index+self.selected_index)%len(self.images)]
            if image_key not in Models.dbImages:      # If no tags exist, create a new list for tags
                Models.dbImages[image_key] = []
            Models.dbImages[image_key].append(self.tagBox.text())    # Append to the tags lis of image
            self.tagBox.setText("")
            self.setFocus()     # Return focus to main window
            self.drawTags()

    def saveTags(self):
        f = open("meta.json", "w+")        # Create file if does not exist
        json.dump(Models.dbImages, f)     # save tags in file
        print("{} saved".format(Models.dbImages))
        f.close()
            ...
            if 5 == 5:      # Checks if there are more than 5 images
                ...
                if center_distance > 0:     # Checks if the selected image is right of center
                    ...
                    for i in range(0, center_distance):
                        ...
                        self.getWidget(4).setPixmap(QPixmap("data/"+self.images[self.circularTraverse(5, "right")]).scaled(((self.W-40)/5)-20, ((self.W-40)/5)-20, Qt.KeepAspectRatio, Qt.SmoothTransformation))
                        self.image_index = self.circularTraverse(1, "right")
                    ...
                    for i in range(0, abs(center_distance)):
                        for j in range(4, 0, -1):
                            self.getWidget(j).setPixmap(self.getWidget(j-1).pixmap().scaled(((self.W-40)/5)-20, ((self.W-40)/5)-20, Qt.KeepAspectRatio, Qt.SmoothTransformation))
                        self.getWidget(0).setPixmap(QPixmap("data/"+self.images[self.circularTraverse(1, "left")]).scaled(((self.W-40)/5)-20, ((self.W-40)/5)-20, Qt.KeepAspectRatio, Qt.SmoothTransformation))
                        self.image_index = self.circularTraverse(1, "left")
                self.select(2)
